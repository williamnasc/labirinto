...
public:
    Coord pos; ///posição
    Coord ant; ///celula anterior
    double g; /// custo passado
    double h; /// custo futuro

    inline noh(): pos(), ant(),g(), h() {}
...
inline double heuristica(Coord pos, Coord dest){
    double h = sqrt(pow((dest.lin-pos.lin),2) + pow((dest.col-pos.col),2));
    return h;
}
...
class menor{
    ...
    double X;
    inline menor():X(-1){}
    inline menor(const noh& N):X(N.g+N.h){}

    inline bool operator()(const noh N){
        double CustoNoh = N.g + N.h;
        if(CustoNoh == X)
            return true;
        else
            return false;
